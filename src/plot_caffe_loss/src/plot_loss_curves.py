...
def create_field_index():
    train_key = 'Train'
    test_key = 'Test'
    field_index = {train_key:{'Iters':0, 'Seconds':1, train_key + ' loss':10, 
                              train_key + ' learning rate':2},
                   test_key:{'Iters':0, 'Seconds':1, test_key + ' acc':3, 
                             test_key + ' lr':2}}
    fields = set()
...
def load_data(data_file, field_idx0, field_idx1):
    data = [[], []]
    data_file_path = osp.join(cwd, data_file)
...
def plot_chart(chart_type, path_to_png, path_to_log_list):
    for path_to_log in path_to_log_list:
        ...
        log_dir = osp.dirname(path_to_log)
        if not log_dir:
            log_dir = './'
        if not osp.exists(path_to_log + '.test') or not osp.exists(path_to_log + '.train'):
            cmd_str = 'python %s %s %s' % (get_log_parsing_script(), path_to_log, log_dir)
            print(cmd_str)
            os.system(cmd_str)
